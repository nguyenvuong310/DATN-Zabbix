...
const App = () => {
  ...
  return (
    <>
      {/* MOBILE SIDE BAR */}
      <div className="sm:hidden fixed top-12 left-0 rounded-tr-full rounded-br-full bg-white p-3 z-40" onClick={toggleSidebar}>
        <GiHamburgerMenu  />
      </div>
      {isAuthenticaed && <PrivacyPolicyModal userId={account.id} hasAccepted={account.policyAccepted} />}
      {/* MOBILE SIDE BAR */}
        <div className={`fixed inset-0 z-50 font-be-vietnam-pro transform transition-transform duration-300 ease-in-out ${isSidebarOpen ? "translate-x-0" : "-translate-x-full"} sm:hidden`}>
          ...
          <div className="absolute flex flex-col justify-between inset-y-0 left-0 w-64 bg-gray-800 p-4 overflow-y-auto">
            ...
            <button className="flex items-center space-x-2 ml-7 mt-2 mb-2" onClick={handleLogout}>
    ...
    <div className="p-4 flex space-x-5 h-svh bg-black">
      ...
      <div className="flex flex-col max-sm:hidden justify-between sticky top-0 h-full">
        ...
        <div className="relative flex max-sm:hidden place-self-center rounded-full bg-gray-800 w-full aspect-1">
          <img src={Logo} alt="logo" className="absolute p-2 inset-0 w-full h-full rounded-full object-cover" />
        </div>
