...
import { togglePolicyAcceptedAction } from '../../redux/action/userAction';

const PrivacyPolicyModal = ({ userId, hasAccepted }) => {
    const [isPop, setIsPop] = useState(false);
    const [isVisible, setIsVisible] = useState(false);
    const dispatch = useDispatch();
    const isPolicyShown = sessionStorage.getItem('isPolicyShown');
    ...
    const handleAccept = () => {
        setIsVisible(false);
        sessionStorage.setItem('isPolicyShown', 'true');
        setTimeout(() => {
            setIsPop(false);
            if (!hasAccepted) {
                dispatch(togglePolicyAcceptedAction(userId));
            }
        }, 300);
    };
    ...
    return (
        isPop && (
            <div className={`fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 font-be-vietnam-pro z-50 transition-opacity duration-300 ease-in-out ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
                <div className={`bg-white p-6 rounded-lg shadow-lg w-11/12 max-sm:h-[70%] max-sm:overflow-auto md:w-[50%] lg:w-[70%] transform transition-transform duration-300 ease-in-out ${isVisible ? 'scale-100' : 'scale-95'}`}>
