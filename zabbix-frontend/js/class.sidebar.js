...
class CSidebar extends CBaseComponent {
  ...
  init() {
    ...
    let max_width =
      sidebar_header.clientWidth +
      parseInt(sidebar_header_style.getPropertyValue('margin-left')) +
  ...
  open() {
    ...
    if (!this._is_opened) {
      ...
      if (
        [SIDEBAR_VIEW_MODE_COMPACT, SIDEBAR_VIEW_MODE_HIDDEN].includes(
          this._view_mode,
        )
      ) {
  ...
  close() {
    ...
    if (this._is_opened) {
      ...
      if (
        ...
        if (
          active_item != null &&
          active_item.parentElement != null &&
  ...
  updateSubmenuPosition(force) {
    const update = () => {
      ...
      if (menu_item) {
        ...
        menu_item &&
          menu_item.getSubmenu().updateRect(menu_item._target, {
            top: document.querySelector('.sidebar-nav').getBoundingClientRect()
              .top,
  ...
  registerEvents() {
    this._events = {
      ...
      mouseleave: () => {
        if (
          !this._is_focused ||
          document.activeElement.parentElement === null ||
      ...
      focusout: (e) => {
        if (
          !this._target.contains(e.relatedTarget) &&
          !this._target.matches(':hover')
        ) {
          this._is_focused = false;
      ...
      expandOver: (item) => {
        !this._is_opened &&
          ZABBIX.MenuMain.getExpanded() === null &&
          item.expandSubmenu();
      },
      ...
      expand: (e) => {
        if (
          this._sidebar_scrollable.scrollHeight >
          this._sidebar_scrollable.clientHeight
        ) {
          setTimeout(() => {
      ...
      _update: (view_mode) => {
        if (view_mode === SIDEBAR_VIEW_MODE_COMPACT) {
          ...
          for (const item of ZABBIX.MenuMain.getItems()) {
            item.hasSubmenu() &&
              item.on('mouseenter', () => this._events.expandOver(item));
          }
          ...
          for (const item of ZABBIX.MenuMain.getItems()) {
            item.hasSubmenu() &&
              item.off('mouseenter', () => this._events.expandOver(item));
          }
        ...
        if (this._sidebar_toggle !== null) {
          if (view_mode === SIDEBAR_VIEW_MODE_HIDDEN) {
            ...
            this._sidebar_toggle.removeEventListener(
              'click',
              this._events.toggle,
            );
          }
        ...
        if (
          [SIDEBAR_VIEW_MODE_FULL, SIDEBAR_VIEW_MODE_HIDDEN].includes(view_mode)
        ) {
        ...
        if (view_mode === SIDEBAR_VIEW_MODE_FULL) {
          ...
          window.removeEventListener(
            'resize',
            this._events.updateSubmenuPosition,
          );
        }
      },
    };
    ...
    this._sidebar_scrollable.addEventListener(
      'scroll',
      this._events.updateSubmenuPosition,
    );
    document
      .querySelector('.wrapper')
      .addEventListener('scroll', this._events.collapseSubmenu);
